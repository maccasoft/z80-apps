...
static void add_uncounted(uint8_t data)
{
...
static void pad()
{
	...
	add_uncounted((uint8_t)(byteCount >> 29)); // Shifting to multiply by 8
	add_uncounted((uint8_t)(byteCount >> 21)); // as SHA-1 supports bitstreams as well as
	add_uncounted((uint8_t)(byteCount >> 13)); // byte.
	add_uncounted((uint8_t)(byteCount >> 5));
	add_uncounted((uint8_t)(byteCount << 3));
}
...
void sha1_init_hmac(const uint8_t *key, int keyLength)
{
	...
	for (i = 0; i < BLOCK_LENGTH; i++) {
		sha1_write_byte(keyBuffer[i] ^ HMAC_IPAD);
	}
}
...
uint8_t * sha1_result_hmac()
{
	...
	init();
	for (i = 0; i < BLOCK_LENGTH; i++) {
		sha1_write_byte(keyBuffer[i] ^ HMAC_OPAD);
	}
	for (i = 0; i < HASH_LENGTH; i++) {
		sha1_write_byte(innerHash[i]);
	}
